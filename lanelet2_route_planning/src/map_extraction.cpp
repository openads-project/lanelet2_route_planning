...
bool GlobalPlanner::extractLocalMapInfo(const perception_msgs::msg::EgoData& ego_data,
  ...
  route_local_tmp.header.frame_id =
      ll2if_->map_frame_id_;  // currently it's map --> will be changed through transform function
  // Check if the found samples are valid
  if (initial_ego_pos_sample_cl_ < route_global.remaining_route.size() &&
      ego_pos_sample_cl <= route_global.remaining_route.size() && ego_pos_sample_cl >= initial_ego_pos_sample_cl_) {
    route_local_tmp.traveled_route = {route_global.remaining_route.begin() + initial_ego_pos_sample_cl_,
                                      route_global.remaining_route.begin() + ego_pos_sample_cl};
  }
  if (ego_pos_sample_cl < route_global.remaining_route.size() &&
      target_pos_sample_cl_ <= route_global.remaining_route.size() && target_pos_sample_cl_ >= ego_pos_sample_cl) {
    route_local_tmp.remaining_route = {route_global.remaining_route.begin() + ego_pos_sample_cl,
                                       route_global.remaining_route.begin() + target_pos_sample_cl_};
  }
  ...
  if (route_local_tmp.remaining_route.size() > 1) {
    ...
    for (size_t i = ego_pos_sample_cl; i < route_global.remaining_route.size(); ++i) {
      if (route_global.remaining_route[i].z - route_global.remaining_route[ego_pos_sample_cl].z >=
          look_ahead_distance) {
        look_ahead_sample = i;
        break;
      }
    ...
    for (size_t i = ego_pos_sample_cl; i >= 0; --i) {
      if (route_global.remaining_route[ego_pos_sample_cl].z - route_global.remaining_route[i].z >=
              look_behind_distance_ ||
          i == 0) {
    ...
    if (look_behind_sample < route_global.remaining_route.size() &&
        look_ahead_sample <= route_global.remaining_route.size() && look_ahead_sample > look_behind_sample) {
      extraction_path = {route_global.remaining_route.begin() + look_behind_sample,
                         route_global.remaining_route.begin() + look_ahead_sample};
    }
    ...
    if (!extraction_path.empty()) {
      ...
      lbehind_sample_rbound_left =
          findNearestSample(extraction_path.front(), route_global.boundaries.left, lbehind_sample_rbound_left);
      lbehind_sample_rbound_right =
          findNearestSample(extraction_path.front(), route_global.boundaries.right, lbehind_sample_rbound_right);
      lahead_sample_rbound_left =
          findNearestSample(extraction_path.back(), route_global.boundaries.left, lahead_sample_rbound_left);
      lahead_sample_rbound_right =
          findNearestSample(extraction_path.back(), route_global.boundaries.right, lahead_sample_rbound_right);
      ...
      // Check if the found samples are valid
      if (lbehind_sample_rbound_left < route_global.boundaries.left.size() &&
          lahead_sample_rbound_left <= route_global.boundaries.left.size() &&
          lahead_sample_rbound_left > lbehind_sample_rbound_left) {
        route_local_tmp.boundaries.left = {route_global.boundaries.left.begin() + lbehind_sample_rbound_left,
                                           route_global.boundaries.left.begin() + lahead_sample_rbound_left};
      } else {
        RCLCPP_WARN_STREAM(get_logger(),
                           "Unable to extract local route boundary. Using global route boundary instead!");
        route_local_tmp.boundaries.left = route_global.boundaries.left;
      }
      if (lbehind_sample_rbound_right < route_global.boundaries.right.size() &&
          lahead_sample_rbound_right <= route_global.boundaries.right.size() &&
          lahead_sample_rbound_right > lbehind_sample_rbound_right) {
        route_local_tmp.boundaries.right = {route_global.boundaries.right.begin() + lbehind_sample_rbound_right,
                                            route_global.boundaries.right.begin() + lahead_sample_rbound_right};
      } else {
        RCLCPP_WARN_STREAM(get_logger(),
                           "Unable to extract local route boundary. Using global route boundary instead!");
      ...
      // Find nearest Boundary-Sample for left and right boundary at look-ahead and look-behind point
      lbehind_sample_drivspace_left = findNearestSample(
          extraction_path.front(), route_global.driveable_space.boundaries.left, lbehind_sample_drivspace_left);
      lbehind_sample_drivspace_right = findNearestSample(
          extraction_path.front(), route_global.driveable_space.boundaries.right, lbehind_sample_drivspace_right);
      lahead_sample_drivspace_left = findNearestSample(
          extraction_path.back(), route_global.driveable_space.boundaries.left, lahead_sample_drivspace_left);
      lahead_sample_drivspace_right = findNearestSample(
          extraction_path.back(), route_global.driveable_space.boundaries.right, lahead_sample_drivspace_right);

      if (lbehind_sample_drivspace_left == route_global.driveable_space.boundaries.left.size() - 1)
        lbehind_sample_drivspace_left = 0;
      if (lbehind_sample_drivspace_right == route_global.driveable_space.boundaries.right.size() - 1)
        lbehind_sample_drivspace_right = 0;
      if (lahead_sample_drivspace_left == 0)
        lahead_sample_drivspace_left = route_global.driveable_space.boundaries.left.size() - 1;
      if (lahead_sample_drivspace_right == 0)
        lahead_sample_drivspace_right = route_global.driveable_space.boundaries.right.size() - 1;

      // Check if the found samples are valid
      if (lbehind_sample_drivspace_left < route_global.driveable_space.boundaries.left.size() &&
          lahead_sample_drivspace_left <= route_global.driveable_space.boundaries.left.size() &&
          lahead_sample_drivspace_left > lbehind_sample_drivspace_left) {
        route_local_tmp.driveable_space.boundaries.left = {
            route_global.driveable_space.boundaries.left.begin() + lbehind_sample_drivspace_left,
            route_global.driveable_space.boundaries.left.begin() + lahead_sample_drivspace_left};
      } else {
        RCLCPP_WARN_STREAM(
            get_logger(),
            "Unable to extract local driveable space boundary. Using global driveable space boundary instead!");
        route_local_tmp.driveable_space.boundaries.left = route_global.driveable_space.boundaries.left;
      }
      if (lbehind_sample_drivspace_right < route_global.driveable_space.boundaries.right.size() &&
          lahead_sample_drivspace_right <= route_global.driveable_space.boundaries.right.size() &&
          lahead_sample_drivspace_right > lbehind_sample_drivspace_right) {
        route_local_tmp.driveable_space.boundaries.right = {
            route_global.driveable_space.boundaries.right.begin() + lbehind_sample_drivspace_right,
            route_global.driveable_space.boundaries.right.begin() + lahead_sample_drivspace_right};
      } else {
        RCLCPP_WARN_STREAM(
            get_logger(),
            "Unable to extract local driveable space boundary. Using global driveable space boundary instead!");
      ...
      double min_x = INFINITY, min_y = INFINITY, max_x = -INFINITY,
             max_y = -INFINITY;  // Parameters describing the rectangle of the area of interest

      // Iterate over left boundary of driveable space
      for (size_t i = 0; i < route_local_tmp.driveable_space.boundaries.left.size(); i++) {
        if (route_local_tmp.driveable_space.boundaries.left[i].x > max_x)
          max_x = route_local_tmp.driveable_space.boundaries.left[i].x;
        if (route_local_tmp.driveable_space.boundaries.left[i].y > max_y)
          max_y = route_local_tmp.driveable_space.boundaries.left[i].y;
        if (route_local_tmp.driveable_space.boundaries.left[i].x < min_x)
          min_x = route_local_tmp.driveable_space.boundaries.left[i].x;
        if (route_local_tmp.driveable_space.boundaries.left[i].y < min_y)
          min_y = route_local_tmp.driveable_space.boundaries.left[i].y;
      }
      // Iterate over right boundary of driveable space
      for (size_t i = 0; i < route_local_tmp.driveable_space.boundaries.right.size(); i++) {
        if (route_local_tmp.driveable_space.boundaries.right[i].x > max_x)
          max_x = route_local_tmp.driveable_space.boundaries.right[i].x;
        if (route_local_tmp.driveable_space.boundaries.right[i].y > max_y)
          max_y = route_local_tmp.driveable_space.boundaries.right[i].y;
        if (route_local_tmp.driveable_space.boundaries.right[i].x < min_x)
          min_x = route_local_tmp.driveable_space.boundaries.right[i].x;
        if (route_local_tmp.driveable_space.boundaries.right[i].y < min_y)
          min_y = route_local_tmp.driveable_space.boundaries.right[i].y;
      }

      // Create a bounding-box (via an area) that encloses the entire local driveable space
      lanelet::LineString3d top(lanelet::utils::getId(), {lanelet::Point3d{lanelet::utils::getId(), max_x, min_y, 0},
                                                          lanelet::Point3d{utils::getId(), max_x, max_y, 0}});
      lanelet::LineString3d right(lanelet::utils::getId(), {lanelet::Point3d{lanelet::utils::getId(), max_x, max_y, 0},
                                                            lanelet::Point3d{utils::getId(), min_x, max_y, 0}});
      lanelet::LineString3d bottom(lanelet::utils::getId(), {lanelet::Point3d{lanelet::utils::getId(), min_x, max_y, 0},
                                                             lanelet::Point3d{utils::getId(), min_x, min_y, 0}});
      lanelet::LineString3d left(lanelet::utils::getId(), {lanelet::Point3d{lanelet::utils::getId(), min_x, min_y, 0},
                                                           lanelet::Point3d{utils::getId(), max_x, min_y, 0}});
      ...
      for (size_t i = 0; i < aoi_lanelets.size(); i++) {
        ...
        lane.centerline = Lanelet2Utilities::convertLaneletLine2Linestring(cur_ll.centerline2d().basicLineString());
        lane.left = deriveLaneSeparator(cur_ll.leftBound2d());
        lane.right = deriveLaneSeparator(cur_ll.rightBound2d());
        route_local_tmp.lanes.push_back(lane);
      }

      // Find all Regulatory Elements within AoI
      std::vector<std::shared_ptr<const lanelet::RegulatoryElement>> aoi_regelems =
          llmap->regulatoryElementLayer.search(aoi_box);
      for (size_t i = 0; i < aoi_regelems.size(); i++) {
        ...
        if (ref_lines.size()) {
          ...
          if (ref_points.size() > 1) {
            ...
            ref_lines_height = regelem.effect_line[0].z;
            regelem.effect_line[0].z = -1.0;
            regelem.effect_line[1].z = -1.0;
          }
        }
        // Get all refering elements
        std::vector<lanelet::ConstLineString3d> refering_elems =
            aoi_regelems[i]->getParameters<lanelet::ConstLineString3d>(RoleName::Refers);
        for (size_t j = 0; j < refering_elems.size(); ++j) {
          std::vector<geometry_msgs::msg::Point> ref_points =
              Lanelet2Utilities::convertLaneletLine2Linestring(refering_elems[j].basicLineString());
          ref_points[0].z -= ref_lines_height;
        ...
        if (aoi_regelems[i]->hasAttribute("subtype")) {
          ...
          if (subtype == "traffic_light")
            regelem.type = route_planning_msgs::msg::RegulatoryElement::TYPE_TRAFFIC_LIGHT;
          ...
          if (subtype == "traffic_sign") {
            if (refering_elems.size() && refering_elems[0].hasAttribute("type") &&
...
unsigned int GlobalPlanner::findNearestSample(const geometry_msgs::msg::Point& ref_point,
...
unsigned int GlobalPlanner::findNearestSampleReverse(const geometry_msgs::msg::Point& ref_point,
                                                     const std::vector<geometry_msgs::msg::Point>& point_list) {
  double min_distance = std::numeric_limits<double>::max();
