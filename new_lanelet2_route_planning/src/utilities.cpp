...
uint8_t laneBoundaryType(const ll::ConstLineString2d& line) {
  ...
  uint8_t lane_boundary_type = route_planning_msgs::msg::LaneBoundary::UNKNOWN; // TODO: rename to TYPE_UNKNOWN?
  ...
  if (line.hasAttribute("type")) {
    ...
    lane_boundary_type = route_planning_msgs::msg::LaneBoundary::UNKNOWN;
    return lane_boundary_type;
  }

  // map lanelet type to lane boundary type
  if (type == "road_boarder" || type == "barrier") {
    lane_boundary_type = route_planning_msgs::msg::LaneBoundary::CROSSING_RESTRICTED;
    ...
    if (line.hasAttribute("subtype")) {
      ...
      if (subtype == "solid" || subtype == "solid_solid") {
        ...
        lane_boundary_type = route_planning_msgs::msg::LaneBoundary::CROSSING_ALLOWED;
      } else if (subtype == "dashed_solid") {
        lane_boundary_type = route_planning_msgs::msg::LaneBoundary::CROSSING_ALLOWED_FROM_LEFT;
      } else if (subtype == "solid_dashed") {
        lane_boundary_type = route_planning_msgs::msg::LaneBoundary::CROSSING_ALLOWED_FROM_RIGHT;
      }
    }
